...
Node* create_node(int n_colors){
    ...
    node->state = 0;
    node->available_colors = malloc(sizeof(int) * n_colors);
    for(int i=0;i<n_colors;i++){
        node->available_colors[i] = i + 1;
    }
    node->available_colors_length = n_colors;
    node->available_colors_max_length = n_colors;
}
